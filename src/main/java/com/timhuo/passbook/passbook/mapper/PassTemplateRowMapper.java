...
/**
 * @description: HBase PAssTemplate Row To PassTemplate Object
 * @author: Tim_Huo
 * @created: 2020/10/08 19:22
 */
public class PassTemplateRowMapper implements RowMapper<PassTemplate> {
    ...
    public PassTemplate mapRow(Result result, int i) throws Exception {
        ...
        passTemplate.setStartTime(DateUtils.parseDate(Bytes.toString(result.getValue(FAMILY_C, START)), patterns));
        passTemplate.setEndTime(DateUtils.parseDate(Bytes.toString(result.getValue(FAMILY_C, END)), patterns));
